...
@Component({
    selector   : 'rfp-shortcuts',
    templateUrl: './shortcuts.component.html',
    styleUrls  : ['./shortcuts.component.scss']
})
export class RfpShortcutsComponent implements OnInit, OnDestroy
{
    shortcutItems: any[];
    navigationItems: any[];
    ...
    ngOnInit(): void
    {
        ...
        this.filteredNavigationItems = this.navigationItems = this._rfpNavigationService.getFlatNavigation(this.navigation);

        if ( this._cookieService.check('RFP2.shortcuts') )
        {
            this.shortcutItems = JSON.parse(this._cookieService.get('RFP2.shortcuts'));
        }
        else
        {
    ...
    toggleShortcut(event, itemToToggle): void
    {
        ...
        for ( let i = 0; i < this.shortcutItems.length; i++ )
        {
            if ( this.shortcutItems[i].url === itemToToggle.url )
            {
                ...
                // Save to the cookies
                this._cookieService.set('RFP2.shortcuts', JSON.stringify(this.shortcutItems));
